...
import net.mcreator.ordeal.OrdealModPlayerAnimationAPI;
...
public abstract class PlayerAnimationMixin<T extends LivingEntity> {
	...
	public void setupPivot(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		...
		if (entityIn instanceof Player player_)
			...
			return;
		OrdealModPlayerAnimationAPI.PlayerAnimation animation = OrdealModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null)
			return;
	...
	public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		...
		if (!master.equals("ordeal")) {
			if (!OrdealModPlayerAnimationAPI.animations.isEmpty())
				OrdealModPlayerAnimationAPI.animations.clear();
			return;
		}
		PlayerModel<T> model = (PlayerModel<T>) (Object) this;
		Player player = null;
		...
		if (data.getBoolean("ResetPlayerAnimation")) {
			...
			OrdealModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		OrdealModPlayerAnimationAPI.PlayerAnimation animation = OrdealModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = OrdealModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				OrdealMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			OrdealModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, OrdealModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			OrdealModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
			ModelPart modelPart = getModelPart(model, boneName);
			if (modelPart == null)
				continue;
			// Apply rotation
			Vec3 rotation = OrdealModPlayerAnimationAPI.PlayerBone.interpolate(bone.rotations, animationProgress, player);
			if (rotation != null) {
			...
			Vec3 position = OrdealModPlayerAnimationAPI.PlayerBone.interpolate(bone.positions, animationProgress, player);
			if (position != null) {
			...
			// Apply scale
			Vec3 scale = OrdealModPlayerAnimationAPI.PlayerBone.interpolate(bone.scales, animationProgress, player);
			if (scale != null) {
