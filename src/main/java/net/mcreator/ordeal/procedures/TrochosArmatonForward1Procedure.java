...
public class TrochosArmatonForward1Procedure {
			...
			if (entity instanceof ServerPlayer || entity instanceof Player) {
				...
				if (entity instanceof Player) {
					if (entity.level().isClientSide()) {
						...
						data.putString("PlayerCurrentAnimation", "ordeal:trochos_armaton_forward");
						data.putBoolean("OverrideCurrentAnimation", true);
						data.putBoolean("FirstPersonAnimation", false);
					} else {
						PacketDistributor.sendToPlayersInDimension((ServerLevel) entity.level(), new PlayPlayerAnimationMessage(entity.getId(), "ordeal:trochos_armaton_forward", true, false));
					}
				}
			}
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
				_entity.addEffect(new MobEffectInstance(OrdealModMobEffects.TROCHOS_ARMATON_FORWARD, 6, 1, false, false));
		}
	}
}