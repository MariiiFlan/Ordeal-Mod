...
public class DescendingMeteor0Procedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (!world.isClientSide()) {
			...
			}
			{
				...
				_vars.abilityName = "Descending Meteor";
				_vars.markSyncDirty();
			}
			cooldownTicks = 170 * (1 - Math.min(0.35, entity.getData(OrdealModVariables.PLAYER_VARIABLES).statAgility * 0.0035));
			Chi_Cost = 10 * (1 - Math.min(0.4, entity.getData(OrdealModVariables.PLAYER_VARIABLES).statChiControl * 0.004));
			BaseDMG = 2.5;
			ExtraDMG = 0.4;
			{
				OrdealModVariables.PlayerVariables _vars = entity.getData(OrdealModVariables.PLAYER_VARIABLES);
					...
					if (!(entity instanceof LivingEntity _livEnt16 && _livEnt16.hasEffect(OrdealModMobEffects.STUNNED))) {
						if (cooldown <= 0) {
							if (entity.getData(OrdealModVariables.PLAYER_VARIABLES).chi >= Chi_Cost) {
								if (!entity.onGround()) {
									DescendingMeteor1Procedure.execute(world, entity);
									if ((entity.getData(OrdealModVariables.PLAYER_VARIABLES).abilityName).equals(entity.getData(OrdealModVariables.PLAYER_VARIABLES).loadout_1)) {
									...
									if (entity instanceof Player _player && !_player.level().isClientSide())
										_player.displayClientMessage(Component.literal(("\u00A74" + "\u00A7l" + "You need to be in the air to use this ability.")), true);
								}
							} else {
								if (entity instanceof Player _player && !_player.level().isClientSide())
