...
public class Akonito2Procedure {
			...
			if (entity instanceof ServerPlayer || entity instanceof Player) {
				...
				if (entity instanceof Player) {
					if (entity.level().isClientSide()) {
						...
						data.putString("PlayerCurrentAnimation", "ordeal:akonito_left");
						data.putBoolean("OverrideCurrentAnimation", true);
						data.putBoolean("FirstPersonAnimation", false);
					} else {
						PacketDistributor.sendToPlayersInDimension((ServerLevel) entity.level(), new PlayPlayerAnimationMessage(entity.getId(), "ordeal:akonito_left", true, false));
					}
				}
				net.mcreator.ordeal.AnimFx.play(entity, "akonito_left");
			}
			if (!((entity instanceof LivingEntity _livEnt ? _livEnt.getOffhandItem() : ItemStack.EMPTY).getItem() == OrdealModItems.AKONTIO.get())) {
				{
					OrdealModVariables.PlayerVariables _vars = entity.getData(OrdealModVariables.PLAYER_VARIABLES);
					_vars.beforeSwap = (entity instanceof LivingEntity _livEnt ? _livEnt.getOffhandItem() : ItemStack.EMPTY).copy();
					_vars.markSyncDirty();
				}
				entity.getData(OrdealModVariables.PLAYER_VARIABLES).beforeSwap.applyComponents((entity instanceof LivingEntity _livEnt ? _livEnt.getOffhandItem() : ItemStack.EMPTY).getComponents());
				{
					OrdealModVariables.PlayerVariables _vars = entity.getData(OrdealModVariables.PLAYER_VARIABLES);
