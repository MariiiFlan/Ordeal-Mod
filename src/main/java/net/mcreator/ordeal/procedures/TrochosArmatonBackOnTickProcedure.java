...
public class TrochosArmatonBackOnTickProcedure {
			...
			}
			{
				...
				for (Entity entityiterator : world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(6 / 2d), e -> true).stream().sorted(Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_center))).toList()) {
					if (!(entityiterator == entity)) {
						...
						entityiterator.setDeltaMovement(new Vec3((entity.getLookAngle().x * entity.getData(OrdealModVariables.PLAYER_VARIABLES).knockback * (-1)), (entity.getDeltaMovement().y()),
								(entity.getLookAngle().z * entity.getData(OrdealModVariables.PLAYER_VARIABLES).knockback * (-1))));
						...
						OrdealMod.queueServerWork(3, () -> {
							if (entity instanceof LivingEntity _entity)
								_entity.removeEffect(OrdealModMobEffects.TROCHOS_ARMATON_BACK);
						});
					}
					{
						Entity _ent = entity;
						if (!_ent.level().isClientSide() && _ent.getServer() != null) {
							_ent.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level() instanceof ServerLevel ? (ServerLevel) _ent.level() : null, 4,
									_ent.getName().getString(), _ent.getDisplayName(), _ent.level().getServer(), _ent), "photon fx photon:ilios_trochosarmaton entity @s 0 1 0 0 0 0 1 1 1 0 false true xrot");
						}
					}
					if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
						_entity.addEffect(new MobEffectInstance(OrdealModMobEffects.SCREEN_SHAKE, 5, 1, false, false));
					if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
						_entity.addEffect(new MobEffectInstance(OrdealModMobEffects.BACK_DASH, 2, (int) (3 * entity.getData(OrdealModVariables.PLAYER_VARIABLES).chargePower), false, false));
				}
			}
		}
	}
