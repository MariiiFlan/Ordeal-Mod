...
public class DecimationClosing0Procedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (!world.isClientSide()) {
			...
			}
			{
				...
				_vars.abilityName = "Decimation Closing";
				_vars.markSyncDirty();
			}
			cooldownTicks = 1200 * (1 - Math.min(0.35, entity.getData(OrdealModVariables.PLAYER_VARIABLES).statAgility * 0.0035));
			Chi_Cost = 55 * (1 - Math.min(0.4, entity.getData(OrdealModVariables.PLAYER_VARIABLES).statChiControl * 0.004));
			BaseDMG = 3.5;
			ExtraDMG = 0.5;
			{
				OrdealModVariables.PlayerVariables _vars = entity.getData(OrdealModVariables.PLAYER_VARIABLES);
					...
					if (!(entity instanceof LivingEntity _livEnt16 && _livEnt16.hasEffect(OrdealModMobEffects.STUNNED))) {
						if (cooldown <= 0) {
							if (entity.getData(OrdealModVariables.PLAYER_VARIABLES).chi >= Chi_Cost) {
								net.mcreator.ordeal.DecimationClosing.start(entity);
								if ((entity.getData(OrdealModVariables.PLAYER_VARIABLES).abilityName).equals(entity.getData(OrdealModVariables.PLAYER_VARIABLES).loadout_1)) {
