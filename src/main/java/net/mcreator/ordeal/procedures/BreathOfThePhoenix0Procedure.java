...
public class BreathOfThePhoenix0Procedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (!world.isClientSide()) {
			...
			}
			{
				...
				_vars.abilityName = "Breath of the Phoenix";
				_vars.markSyncDirty();
			}
			cooldownTicks = 300 * (1 - Math.min(0.35, entity.getData(OrdealModVariables.PLAYER_VARIABLES).statAgility * 0.0035));
			Chi_Cost = 14 * (1 - Math.min(0.4, entity.getData(OrdealModVariables.PLAYER_VARIABLES).statChiControl * 0.004));
			BaseDMG = 3.5;
			ExtraDMG = 0.4;
			{
				OrdealModVariables.PlayerVariables _vars = entity.getData(OrdealModVariables.PLAYER_VARIABLES);
			...
			mode = 1;
			chiPerTick = 0.05;
			ThirdPersonAni = "breath_of_phoenix";
			FirstPersonAni = "";
			Projectile = true;
						...
						if (cooldown <= 0) {
							if (entity.getData(OrdealModVariables.PLAYER_VARIABLES).chi >= Chi_Cost) {
								if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
									_entity.addEffect(new MobEffectInstance(OrdealModMobEffects.BREATHOF_PHOENIX, 100, 1, false, false));
								{
									OrdealModVariables.PlayerVariables _vars = entity.getData(OrdealModVariables.PLAYER_VARIABLES);
