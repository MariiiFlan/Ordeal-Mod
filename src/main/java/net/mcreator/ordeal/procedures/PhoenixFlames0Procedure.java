...
public class PhoenixFlames0Procedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (!world.isClientSide()) {
			...
			}
			{
				...
				_vars.abilityName = "Phoenix Flames";
				_vars.markSyncDirty();
			}
			cooldownTicks = 60 * (1 - Math.min(0.35, entity.getData(OrdealModVariables.PLAYER_VARIABLES).statAgility * 0.0035));
			Chi_Cost = 8 * (1 - Math.min(0.4, entity.getData(OrdealModVariables.PLAYER_VARIABLES).statChiControl * 0.004));
			BaseDMG = 4;
			ExtraDMG = 0.4;
			{
				OrdealModVariables.PlayerVariables _vars = entity.getData(OrdealModVariables.PLAYER_VARIABLES);
			...
			FirstPersonAni = "ordealanimations play @s shoot1_hold";
			Projectile = true;
			ExplodeOnImpact = true;
			ExploVFX = "ilios_explosions";
			ExplodeRadius = 3;
			IgniteSeconds = 3;
			Homing = 0.5;
								...
								if (entity instanceof ServerPlayer || entity instanceof Player) {
									if (entity instanceof Player) {
										if (entity.level().isClientSide()) {
											...
											data.putString("PlayerCurrentAnimation", "ordeal:phoenix_flames_hold_ground");
											data.putBoolean("OverrideCurrentAnimation", true);
											data.putBoolean("FirstPersonAnimation", true);
										} else {
											PacketDistributor.sendToPlayersInDimension((ServerLevel) entity.level(), new PlayPlayerAnimationMessage(entity.getId(), "ordeal:phoenix_flames_hold_ground", true, true));
										}
									}
								}
								PhoenixFlames1Procedure.execute(world, entity);
								if ((entity.getData(OrdealModVariables.PLAYER_VARIABLES).abilityName).equals(entity.getData(OrdealModVariables.PLAYER_VARIABLES).loadout_1)) {
