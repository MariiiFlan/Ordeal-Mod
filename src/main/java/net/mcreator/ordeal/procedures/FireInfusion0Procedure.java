...
public class FireInfusion0Procedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (!world.isClientSide()) {
			...
			}
			{
				...
				_vars.abilityName = "Fire Infusion";
				_vars.markSyncDirty();
			}
			cooldownTicks = 0 * (1 - Math.min(0.35, entity.getData(OrdealModVariables.PLAYER_VARIABLES).statAgility * 0.0035));
			Chi_Cost = 4 * (1 - Math.min(0.4, entity.getData(OrdealModVariables.PLAYER_VARIABLES).statChiControl * 0.004));
			BaseDMG = 0;
			ExtraDMG = 0.5;
			{
				OrdealModVariables.PlayerVariables _vars = entity.getData(OrdealModVariables.PLAYER_VARIABLES);
